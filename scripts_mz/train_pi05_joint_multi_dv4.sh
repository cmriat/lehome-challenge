#!/bin/zsh
# Pi0.5 Joint Action Multi-node Training Script for LeHome Challenge
# Usage: pixi run submit-multi joint 2 scripts_mz/train_pi05_joint_multi.sh [NUM_GPUS_PER_NODE]
...
CONFIG_PATH="configs/train_pi05_joint_raw_reject_v4.yaml"
...
LOG_FILE="${LOG_DIR}/train_pi05_joint_multi_node${NODE_RANK}_${TIMESTAMP}.log"
mkdir -p "$LOG_DIR"

exec > >(stdbuf -oL -eL tee -a "$LOG_FILE") 2>&1

echo "=============================================="
echo "Pi0.5 Joint Action Multi-node Training: LeHome Challenge"
...
echo "MACHINE_RANK=${MACHINE_RANK:-unset}"
...
echo "SLURM_JOB_FIRST_NODE_IP=${SLURM_JOB_FIRST_NODE_IP:-unset}"
